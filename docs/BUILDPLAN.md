...
- **Pagination:** cursor-based. The response's `links.next` is a path **relative to `/v2`** (e.g. `/userCollectionTracks/...?page[cursor]=...`). `fetchLibraryPage` prepends `TIDAL_API_BASE` only if the path doesn't already start with `http`. Tests assume the same shape. **Gotcha found in 1c.1 (2026-05-27):** the cursor is unstable over the non-unique default `sort=-addedAt` — at tie-clusters of equal keys it re-emits some tracks and skips others (~95 of 521 lost). Mitigated by unioning passes over multiple sort orders; see the 2026-05-27 decision-log entry.
...
- **Subrequest cap breached in prod → resolved in Phase 1c.1 (2026-05-27).** As predicted, a real 521-track library exceeded the free-plan 50-subrequest cap once the under-fetch fix required a multi-sort union (~55 calls → `Error: Too many subrequests`). The flagged "cursor resumption" path was taken: `/sync` now fetches one sort order per request and the client loops the passes. Details in the 2026-05-27 decision-log entry.
- **Manual end-to-end verification done (2026-05-27).** Phase 1d's UI sync was finally exercised against the owner's real library — which is what surfaced the cursor under-fetch (426 of 521) and the subrequest-cap 500, both fixed in 1c.1. Count now reaches 521 and holds across reloads. Remote D1 is now seeded with the owner's library.
- **Known limit — large libraries (deferred).** The 1c.1 design splits *passes* across requests but not a *single* pass, so one sort pass must fit the free-plan 50-subrequest cap: `1 + ceil(size/20) ≤ 50` → **~950 tracks max** (less under 429s). The owner's real library is ~5000 tracks (in Apple Music — a v2+ tier), so this *will* need solving before it serves that collection, or if the TIDAL library grows past ~900. Fix when needed: **intra-pass cursor resumption** (checkpoint the page cursor in KV/D1 keyed by syncId, resume in bounded chunks — removes the ceiling), or the zero-code stopgap of **Workers Paid** (1000-subrequest cap → ~20k ceiling). Not built; flagged so a large-library sync doesn't 500 unexpectedly.
...
**Files created/modified (actuals, 2026-05-26 — code complete, live verification pending):**
- `musicbot/package.json` — added `@tidal-music/player` (`0.18.1`). Self-contained (zero runtime deps; bundles shaka). No `@tidal-music/auth` needed — see auth-handoff decision.
- `musicbot/src/db/schema.sql` — added append-only `listen_events` (`id`, `user_id`, `song_id`, `kind` `play_complete|skip|repeat`, `title`, `artist`, `ms_played`, `created_at`) + `idx_listen_events_user`.
- `musicbot/src/routes/playback.ts` (new, mounted at `/api/playback`) — `GET /token` (reuses `refreshIfNeeded`; returns `{accessToken, expiresAt, clientId, userId}` — clientId is public) and `POST /listen` (validate-then-insert, mirrors `feedback.ts`), both behind `requireSession`.
- `musicbot/src/client/lib/api.ts` — `getTidalToken`, `sendListenEvent`; `tidalTrackUrl` retained as the fallback/Phase-5 helper.
- `musicbot/src/client/lib/credentials.ts` (new) — thin custom `CredentialsProvider` (`{bus, getCredentials}`) backed by `/api/playback/token`, caches until ~30s pre-expiry. No browser OAuth.
- `musicbot/src/client/lib/player.ts` (new) — `createPlayerStore(engine, deps)` (owns the rec queue, drives `load`/`setNext`/advance, classifies listen signal), `PlayerProvider`/`usePlayer` (context + `useSyncExternalStore`), plus a no-op default store and a deep-link `createFallbackStore` for SDK-init failure.
- `musicbot/src/client/lib/tidalEngine.ts` (new) — the **only** module importing `@tidal-music/player`; wraps it behind `PlayerEngine`. Dynamically imported by `PlayerProvider` so the SDK never enters unit tests / the main bundle (it's a lazily-loaded ~271 KB-gz chunk).
- `musicbot/src/client/components/Player.tsx` (new) — persistent mini-player bar (now-playing, prev/play-pause/next, scrubber, repeat/shuffle with `aria-pressed`+icon swap, deep-link fallback when `available===false`). 44px targets.
- `musicbot/src/client/main.tsx` — `<PlayerProvider>` wraps `<App/>` + root-mounted `<Player/>` (above the router) so audio + the bar survive tab/route changes.
- `musicbot/src/client/components/RecommendationCard.tsx` — Play is now a `<button onClick={onPlay}>` (enqueue via the store) instead of the deep-link `<a>`; disabled for unresolved recs.
- `musicbot/src/client/pages/Chat.tsx` — wires `onPlay={(r) => playQueue(recs, recs.indexOf(r))}`; `pb-28` clears the fixed bar.
- Tests: `test/playback.spec.ts` (7), `src/client/lib/player.spec.ts` (8), `src/client/components/Player.spec.tsx` (6), and the `RecommendationCard` Play test repointed onto `onPlay`.

**Notes for future sessions:**
- **Auth handoff (the PRD §7 risk):** server token endpoint + a hand-rolled `CredentialsProvider`, *not* `@tidal-music/auth`. The browser never runs OAuth or holds the secret; the token is refreshed server-side (confidential client) and handed over by `/api/playback/token`. If the SDK rejects the custom provider live, fall back to client-side `@tidal-music/auth` (public-client + re-login).
- **SDK model is `load`+`setNext` (no native queue), so the store owns the queue.** `repeat` is **repeat-one** (loops current via `setNext(current)`) — chosen so the "repeat" listen signal is meaningful. The SDK fires a `media-product-transition` for our *own* `load()` too, so the store tracks `ignoreTransitionFor` to not misread that echo as a natural advance.
- **Listen-signal classification** (`player.ts`): a natural transition to the queued-next = `play_complete`; a manual `next()`/`prev()` uses a 90%-played threshold (`play_complete` vs `skip`, ~1–2s precision per PRD §6); a repeat-one loop = `repeat`. Posted best-effort to `/api/playback/listen`.
- **`@tidal-music/player@0.18.1` real API (verified against installed `.d.ts`):** `bootstrap({outputDevices, players:[{itemTypes:["track"], player:"browser"|"shaka"}]})` (takes a **required** config), `load(mp)→Promise`, `play()→Promise`, `pause()→void` (not a promise!), `seek(s)→Promise`, `setNext(mp?)`, `getMediaProduct()`, `getPlaybackContext()?.actualDuration` (duration source), `getPlaybackState()`, `events` is a real `EventTarget` firing `playback-state-change`/`media-product-transition`/`error`. `MediaProduct` needs `{productId, productType, sourceType, sourceId}` — we send `sourceType:"MUSICBOT"`. **`skipLibCheck` saves the build** from the SDK's type-only `@tidal-music/common` import (not installed).
- **NOT yet verified live (needs a subscribed account):** the credentials-provider acceptance, the `bootstrap` player config, the `sourceType` value, and whether full-track playback needs a wider OAuth scope than the current `user.read collection.read collection.write` (may force a `TIDAL_SCOPES` widen + re-login). The `error`-event → deep-link fallback is the safety net.

**Done-when (revised — full-track playback blocked by TIDAL; ships as 30s previews via client-credentials):**
- [x] Play starts in-app audio (**30s preview** — TIDAL's web ceiling, see decision log + [[project-tidal-web-playback-limit]]); **preview playback verified live (Safari, 2026-05-27)**. Getting there took fixing three SDK-integration bugs found live: the Firefox reload loop (token fetch redirected to /login), `play()` called before `load()` resolved ("No active player"), and the SDK's event-sender gate ("Playback not allowed without an event sender" → satisfied with a no-op `setEventSender` stub).
- [x] Persistent mini-player bar mounted at the app root (survives tab/route switches by construction); controls + scrubber covered by `Player.spec.tsx`.
- [x] Full-track access: "Full track on TIDAL ↗" deep link in the bar while previewing; SDK-init failure → deep-link `createFallbackStore`; per-track `error` event → fallback link.
- [x] In-app listen signal captured (`listen_events` + `POST /api/playback/listen`; classification unit-tested — a "play_complete" now means the 30s preview was heard through).
- [x] Tests pass (102/102), typecheck + build clean. **Deployed 2026-05-26** to https://musicbot.musicbot-cs.workers.dev (latest version `c86f19e3`); remote D1 migrated (`listen_events` — 6 tables). Smoke: `/api/health` 200, `/api/playback/token` 401 without a session. Firefox reload loop fixed (token fetch no longer redirects to /login). **Live in-browser preview playback pending owner verification.**

**Session budget:** 2–3 (bumped from 1–2 — full controls + queue + SDK auth handoff; PRD §7's flagged biggest risk).

**Risks / unknowns:** PRD §7's biggest risk. SDK auth handoff (custom provider — fallback `@tidal-music/auth`); subscription gating for full-track playback (deep-link fallback wired); the `bootstrap` config + `sourceType` + possible scope widen are the unverified live pieces. **Out of scope even here:** catalog/search browsing — we build a player, not a Tidal replacement.
...
| 2026-05-26 | Phase 4 | Genre/era handled by LLM inference + a wider library window, not stored metadata | Follow-on to the "no genres/eras" entry. Rather than build a catalog-enrichment pipeline (subrequest wall, new columns, possible Queues), `TOP_ARTISTS` 40→150 / `MAX_SUMMARY_CHARS` 3000→6000 give the model enough breadth to infer genre/era clusters itself, and a system-prompt clause tells it to reason about genres/scenes/eras and "stretch along one axis" for "like X but different" requests. Near-zero cost (BYOK tokens; artist names are tiny). Targets the baseline's `Discovery 1`; effectiveness still pending the live re-score. Structured metadata stays an option if inference proves too unreliable on obscure deep cuts. |
...
| 2026-05-26 | Phase 3.5 | Auth handoff = server token endpoint + thin custom `CredentialsProvider`, not client-side `@tidal-music/auth` | Keeps the OAuth client secret + token refresh server-side (where they already work), adds no browser re-login, and adds only `@tidal-music/player`. The SDK's `CredentialsProvider` is just `{bus, getCredentials}`, so a hand-rolled provider backed by `/api/playback/token` satisfies it. Owner-confirmed. Fallback if the SDK rejects it live: client-side `@tidal-music/auth` (public-client registration + re-login). |
| 2026-05-26 | Phase 3.5 | In-app listen signal → new `listen_events` table, not `feedback_events` | Keeps passive playback signal (full plays / early skips / repeats) distinct from explicit like/dislike/add, so Phase 4's taste profile (which aggregates `feedback_events` as explicit signal) isn't polluted and can opt into `listen_events` later. Owner-confirmed. |
| 2026-05-26 | Phase 3.5 | `repeat` is repeat-one (loops current), not repeat-all | Makes the "repeat" listen signal meaningful (each loop = a strong positive). Implemented via `setNext(current)`; manual next/prev still navigate the queue linearly. Revisit if repeat-all is wanted in the UI. |
| 2026-05-27 | Phase 1c.1 | Library sync reworked into a **resumable multi-sort union with mirror-delete** | Exercising the sync against the owner's real library exposed two bugs the test-only 1c missed. (1) **Under-fetch:** TIDAL's collection cursor pages over the non-unique default `sort=-addedAt` and, at tie-clusters of equal timestamps, re-emits ~95 tracks and skips ~95 — so a sync captured only 426 of 521 (and the count drifted, since the upsert-only sync accumulated a growing union and `/sync` reported the raw fetched length, not the distinct count). Fix = union passes over orthogonal sorts (`-addedAt` → `title` → `artists.name`), stopping when the union matches TIDAL's `numberOfItems`. (2) **Subrequest cap:** ≥2 full passes ≈ 55 calls breach the free-plan 50-cap (`Error: Too many subrequests`). Fix = `/sync` fetches ONE sort per request (`?pass=N`, ~28 calls), stamping rows with a client-threaded `syncId`; the client loops `nextPass` echoing `syncId`. When the union (`COUNT WHERE synced_at=syncId`) reaches `numberOfItems` the fetch is complete and rows with an older `synced_at` are swept — the true two-way mirror the owner asked for, gated so a partial fetch never deletes. Owner chose this over Workers Paid ($5/mo, 1000-cap). `fetchAllLibrary` replaced by exported `fetchCollectionSize` + `fetchLibrarySort` + `LIBRARY_SORTS`. Verified live: 521 captured, stable across reloads. |
| 2026-05-26 | Phase 3.5 | Real SDK isolated in `tidalEngine.ts` behind a `PlayerEngine` interface, dynamically imported | `@tidal-music/player` can't run in happy-dom and is a ~271 KB-gz chunk. Wrapping it lets the store be unit-tested against a fake engine, keeps the SDK out of the main bundle (lazy chunk, loaded when `PlayerProvider` mounts), and confines the unverified-live surface (init order, `sourceType`, event names) to one file. |
| 2026-05-26 | Phase 3.5 / PRD §3 | **Full-track in-app playback is impossible on web** — pivoted to 30s previews (client-credentials) + full-track deep link | Live testing surfaced a hard TIDAL constraint (see [[project-tidal-web-playback-limit]]): the web SDK only streams full tracks with `r_usr`+`playback` scopes, which TIDAL lists "Coming Soon!" and does **not** grant third-party apps (maintainer-confirmed, tidal-sdk-web #133; still unshipped in 2026). A user (auth-code) token fails with "missing required scope" → 0:00/0:00. **Verified 2026-05-26** that a client-credentials *app* token streams a 30s preview (real `audio/mp3` manifest; `assetpresentation=FULL` is silently downgraded to PREVIEW; works without a subscription). So `/api/playback/token` now mints an app token (`clientCredentialsToken`, KV-cached ~4h) and the player plays previews; a "Full track on TIDAL ↗" deep link covers the full song. PRD §3's "client-grade full-track playback" is **blocked by the platform**, not the code — revisit if/when TIDAL ships web full-playback. |
| 2026-05-26 | Phase 3.5 | Card Play switched from deep-link `<a>` to `onPlay` button; deep link moved to the player bar | Play now enqueues into the in-app player. The "Listen on TIDAL" deep link survives as the no-subscription / SDK-failure fallback (player bar + `createFallbackStore`), so there's never a dead Play button. |
