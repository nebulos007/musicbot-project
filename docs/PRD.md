...
- **A first-class v1 experience (revised 2026-05-26):** Users can play and enjoy music directly in the app, not just in Tidal — with real, **client-grade** playback (play/pause, skip, scrubber, a queue built from recommendations, repeat/shuffle), audio that keeps playing uninterrupted as you move around the app. Every in-app listen *also* feeds the taste profile (full plays, early skips, repeats). The v1 *success metric* stays "songs added to library," but enjoyable in-app listening is now an explicit goal, not merely signal capture. _(Reverses the earlier "minimal player / signal-only" stance — see BUILDPLAN decision log.)_
- **Still not a goal for v1:** Replacing Tidal as a catalog/search/library-management client. We build a great *player* for the music we surface, but discovery + feedback remain the app's purpose — we don't reimplement Tidal's full browse/search UX.
